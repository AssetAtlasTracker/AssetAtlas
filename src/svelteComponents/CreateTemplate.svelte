<script lang="ts">
  ...
  debounceTimeout = setTimeout(async () => {
    ...
    try {
      const response = await fetch(`http://${$ip}/api/templates/searchTemplates?name=${encodeURIComponent(name)}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      });
      const data = await response.json();
      //Check if is an EXACT match
      const exactMatch = data.some((template: { name: string }) => template.name === name.trim());
  ...
  <div class="template-container">
    ...
    <form on:submit|preventDefault={handleCreateTemplate}>
        ...
        <div class="flex items-start mb-2 relative">
          ...
          <label class="flex-grow mr-2 relative">
            ...
            {#if field.suggestions.length > 0}
                <ul class="suggestions bg-white border rounded shadow mt-1 max-h-32 overflow-auto absolute z-10 w-full">
          ...
          <!-- Remove Button -->
          <button type="button" class="border-button hover:bg-gray-100 font-semibold shadow ml-2" on:click={() => removeCustomField(index)}>
            Remove
          </button>
      ...
      <button type="submit" class="border-button hover:bg-gray-100 font-semibold shadow mt-4" disabled={!!nameError}>Create Template</button>
    </form>
  </div>
