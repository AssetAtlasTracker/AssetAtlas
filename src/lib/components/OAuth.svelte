<script lang="ts">
	...
	async function checkForAccount() {
		try {
			const response = await fetch(
				`/api/authenticator/checkAccount?username=${encodeURIComponent(username)}`,
			);
			const data = await response.json();
	...
	async function fetchQRCode() {
		try {
			...
			if (!data.qrCode) {
				oauthResult =
					"Existing account found, please enter code from app";
	...
	async function verifyAuthCode() {
		const response = await fetch(
			`/api/authenticator/verify?username=${encodeURIComponent(username)}&code=${encodeURIComponent(authCode)}`,
		);
		const data = await response.json();
...
<dialog
	bind:this={dialog}
	class="glass border fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
>
	<div class="flex flex-col space-y-4 p-4 relative">
		<button
			class="x-button absolute top-0 right-0 mt-2 mr-2"
		...
		{#if !showAuthenticatorLogin}
			<div>
				...
				<button
					class="border-button w-full"
					on:click={handleLoginGoogle}
				>
				...
				<button
					class="border-button w-full"
					on:click={handleAuthenticatorApp}
				>
					...
					<div class="text-center mb-4">
						{#if qrCode}
							<img
								...
								class="mx-auto mb-4"
							/>
							<div class="border p-2 w-full mb-4">
